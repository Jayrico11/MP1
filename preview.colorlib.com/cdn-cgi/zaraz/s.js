    ...
    (function(w, d) {
        ...
        zaraz.i = function(ek) {
            ...
            for (let en = 0; en < em.length; en++) {
                ...
                for (const ep of em[en].attributes) eo.setAttribute(ep.name, ep.value);
        ...
        for (const {
                ...
                m: eN
            }) => ["debug", "set"].includes(eN)))) zaraz[eL](...eM);
        for (const {
                m: eO,
                a: eP
            } of zaraz.q) zaraz[eO](...eP);
        delete zaraz.q;
        delete zarazData.q;
        zaraz.spaPageview = () => {
        ...
        zaraz._processDataLayer = dc => {
            for (const dd of Object.entries(dc)) zaraz.set(dd[0], dd[1], {
                scope: "page"
            });
            if (dc.event) {
                if (zarazData.dataLayerIgnore && zarazData.dataLayerIgnore.includes(dc.event)) return;
                let de = {};
                for (let df of dataLayer.slice(0, dataLayer.indexOf(dc) + 1)) de = { ...de,
                    ...df
                };
                delete de.event;
        ...
        zaraz._p({
            ...
            "f": [
                ["https://stats.g.doubleclick.net/g/collect?t=dc&aip=1&_r=3&v=1&_v=j86&tid=G-SEKJ4E9T4H&cid=c4f83972-e347-4160-9fcc-500709cacbee&_u=KGDAAEADQAAAAC%7E&z=1254233742", {}]
            ]
        })
    })(window, document)
} catch (e) {
